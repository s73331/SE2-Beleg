...
public class PropertyHelper {
    ...
    public PropertyHelper(String file) throws IOException {
        ...
        if(!properties.containsKey("sleeptime")) throw new IOException("key sleeptime not found");
        if(!properties.containsKey("maxmainttime")) throw new IOException("key maxmainttime not found");
        if(!isIntParsable(properties.getProperty("registertimeout"))) throw new IOException("key registertimeout can not be parsed to int");
        if(!isIntParsable(properties.getProperty("taskindconfirmtimeout"))) throw new IOException("key taskindconfirmtimeout can not be parsed to int");
        if(!isIntParsable(properties.getProperty("taskreqtimeout"))) throw new IOException("key taskreqtimeout can not be parsed to int");
        if(!isIntParsable(properties.getProperty("sleeptime"))) throw new IOException("key sleeptime can not be parsed to int");
